...
def test_executor_invalid_train_backend_fails_fast(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(executor_graph_module, "inquirer_agent", lambda state, metrics: {})
    monkeypatch.setattr(executor_graph_module, "dataset_preparer_agent", lambda state: {})
    monkeypatch.setattr(executor_graph_module, "dag_init_agent", lambda state: {})
    monkeypatch.setattr(executor_graph_module, "tspn_bootstrap_agent", lambda state: {})
    monkeypatch.setattr(executor_graph_module, "report_agent_node", lambda state: {})
