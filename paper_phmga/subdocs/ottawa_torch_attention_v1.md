...
`ottawa_torch_attention_v1` is a `ablation` run on `Ottawa` / `torch` with backend `pending / pending`. Canonical status is `pending` from the result ledger.
...
- source_output_dir: `artifacts/paper/ottawa_torch_attention_v1`
