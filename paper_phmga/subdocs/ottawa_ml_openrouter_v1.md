...
bundle_evidence_dir: evidence/ottawa_ml_openrouter_v1
---

# Summary

`ottawa_ml_openrouter_v1` is a `backend_comparison` run on `Ottawa` / `ml` with backend `openrouter / stepfun/step-3.5-flash:free`. Canonical status is `reject` from the result ledger.
...
- source_output_dir: `artifacts/paper/ottawa_ml_openrouter_v1`
