...
def _normalize_h5_array(array: np.ndarray) -> np.ndarray:
    ...
    # PHM-Vibench stores signals as (L, C[, 1]); the internal pipeline expects (C, L).
    return np.asarray(array, dtype=float).T


class SampleMeta(BaseModel):
    """Canonical sample metadata shared by workflow, bridge, and reports."""
...
class SplitManifest(BaseModel):
    """Formal split contract; no legacy aliases are allowed here."""
...
class WindowSpec(BaseModel):
    """Window slicing contract carried into artifacts and reports."""
...
class DatasetProtocol(BaseModel):
    """Dataset-level protocol object used across workflow, bridge, and training."""
    ...
    def ensure_split_ids_exist(self) -> "DatasetProtocol":
        """Guarantee that split manifests only reference known samples."""
...
def _unique_ids(split_cfg: Dict[str, Any]) -> List[str]:
    ...
    seen: set[str] = set()
    for split_name in ("train_ids", "val_ids", "test_ids"):
        for sample_id in split_cfg.get(split_name, []):
            text = str(sample_id)
            if text not in seen:
...
def _resolve_fixed_per_class_splits(label_to_ids: Dict[int, List[str]], split_cfg: Dict[str, Any]) -> SplitManifest:
...
def _resolve_real_splits(filtered_df: pd.DataFrame, split_cfg: Dict[str, Any]) -> SplitManifest:
    ...
    if strategy == "stratified_fixed_per_class":
        return _resolve_fixed_per_class_splits(label_to_ids, split_cfg)
    raise ValueError(f"Unsupported split strategy: {strategy}")


def _build_synthetic_protocol(data_cfg: Dict[str, Any]) -> DatasetProtocol:
    synthetic_cfg = dict(data_cfg.get("synthetic", {}))
    split_cfg = dict(data_cfg.get("splits", {}))
    labels = dict(data_cfg.get("labels", {}))
    all_ids = _unique_ids(split_cfg)
    channels = int(synthetic_cfg["channels"])
    selected_channels = data_cfg.get("selected_channels")
    effective_channels = len(selected_channels) if selected_channels else channels
    samples: list[SampleMeta] = []
    for sample_id in all_ids:
        samples.append(
            SampleMeta(
                ...
                sampling_rate=int(synthetic_cfg["sampling_rate"]),
                length=int(synthetic_cfg["length"]),
                channels=effective_channels,
                operating_condition=str(synthetic_cfg["operating_condition"]),
                source_h5="",
                observed_length=int(synthetic_cfg["length"]),
                observed_channels=effective_channels,
            )
        )
    return DatasetProtocol(
        dataset_name=str(data_cfg["dataset_name"]),
        catalog=str(data_cfg.get("catalog", "synthetic")),
        metadata_schema_version=str(data_cfg.get("metadata_schema_version", "synth_v1")),
        samples=samples,
...
def _build_real_protocol(data_cfg: Dict[str, Any]) -> DatasetProtocol:
        ...
        for row in filtered_df.itertuples(index=False):
            ...
            samples.append(
                SampleMeta(
                    ...
                    file_name=_clean_text(getattr(row, "File", None)),
    ...
    return DatasetProtocol(
        dataset_name=str(data_cfg["dataset_name"]),
        catalog=str(data_cfg.get("catalog", "PHM-Vibench")),
        metadata_schema_version=str(data_cfg.get("metadata_schema_version", "v1")),
        samples=samples,
...
def build_protocol_from_config(config: Dict[str, Any]) -> DatasetProtocol:
    """Build the canonical protocol object from the resolved config payload."""
    data_cfg = dict(config.get("data", {}))
    has_real_paths = bool(data_cfg.get("metadata_path")) and bool(data_cfg.get("h5_path"))
    if has_real_paths:
        return _build_real_protocol(data_cfg)
    return _build_synthetic_protocol(data_cfg)


def _seed(sample_id: str, dataset: str) -> int:
    """Derive a stable seed so synthetic smoke runs are reproducible."""
    seed_value = 0
...
def _generate_signal(sample: SampleMeta) -> np.ndarray:
    """Generate deterministic synthetic signals for offline smoke and unit tests."""
    ...
    base_freq = 5.0 if "RM101" in sample.dataset else 7.0
...
def _window_signal(signal: np.ndarray, window: WindowSpec) -> list[np.ndarray]:
    """Window a signal after split assignment to preserve leakage boundaries."""
