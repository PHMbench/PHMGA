...
2. 依次撰写五个部分：
   ...
   - 模型评估：
     - 若存在 `ml_results.metrics_markdown`（shallow ML），嵌入该表并突出 `ensemble_metrics`。
     - 若存在 `ml_results.tspn.metrics_markdown`（TSPN），嵌入该表并解释其与可解释性证据（operator importance / WF 参数）的对应关系。
