"""Minimal demo pipeline using the operator framework.

This module defines a very small LangGraph that showcases how the
self‑describing operators can be composed into a plan and executed to
classify a test signal.  It is intentionally lightweight so that the unit
tests run quickly and users can easily inspect the flow.
"""
...
from src.tools.schemas import PatchOperator, MeanOperator, SimilarityOperator, PHMOperator
from src.tools.factory import create_operator

class DemoState(TypedDict):
    """Simple state container for the demo graph."""
...
def generate_signals() -> DemoState:
    """Generate random signals and a basic processing plan."""
...
def planner(state: DemoState) -> DemoState:
    """Entry node: simply return the initial state."""

    return state


def tool_executor(state: DemoState) -> DemoState:
    """Apply the plan to both test and reference signals."""
    ...
    # Process each reference signal in the same way
...
def decide(state: DemoState) -> DemoState:
    """Choose the label whose processed signal is most similar."""
...
def build_graph() -> StateGraph:
    """Construct the demonstration graph."""
...
def main() -> None:
    """Run the demo graph and print the result."""
