...
| **方差 (Var)**                  | 
$$
\sigma^2 = \frac{1}{L}\sum_{i=1}^{L} (x_i - \mu)^2
$$
...
$$
                                                        ...
                                                        |
| **绝对值均值 (AbsMean)**        | $$\text{abs\_mean}(x) = \frac{1}{L}\sum_{i=1}^{L}                                |
| **均方根 (RMS)**                | 
$$
\text{rms}(x) = \sqrt{\frac{1}{L}\sum_{i=1}^{L} x_i^2}
$$

                        |
| **偏度 (Skewness)**             | 
$$
\text{skewness}(x) = \frac{\frac{1}{L}\sum_{i=1}^{L} (x_i - \mu)^3}{\sigma^3}
$$

 |
| **峰度 (Kurtosis)**             | 
$$
\text{kurtosis}(x) = \frac{\frac{1}{L}\sum_{i=1}^{L} (x_i - \mu)^4}{\sigma^4}
$$
...
$$
           ...
           |
| **峰值因子 (Crest Factor)**     | $$\text{crest\_factor}(x) = \frac{\max_{i}                                       |
| **裕度因子 (Clearance Factor)** | $$\text{clearance\_factor}(x) = \frac{\max_{i}                                   |
| **峰峰值 (Peak-to-Peak)**       | 
$$
\text{p2p}(x) = \max(x) - \min(x)
$$

                                             |
| **过零率 (Zero-Crossing Rate)** | $$\text{zcr}(x) = \frac{1}{2(L-1)}\sum_{i=1}^{L-1}                               |
